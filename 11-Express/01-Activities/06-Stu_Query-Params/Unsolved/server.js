// Unsolved/server.js (ADD COMMENTS ONLY — DO NOT CHANGE LOGIC)
...
app.get('/api/terms/', (req, res) => {
  ...
  // Check and see if there is a ??? parameter at all
  const hasQuery = Object.keys(req.query).length > 0;

  // If we have a query of 'sort' and it's value is '???' send the results in ??? order
...
app.get('/api/term/:term', (req, res) => {
  ...
  // Iterate through the terms name to check if it matches `???`
  if (requestedTerm) {
